...
(function() {
	...
	$('#search-button').click(function() {
		$("#tables").setGridParam({
			postData: {
				category_name_search: $.trim($('#category_name_search').val())
			},
			page:1
		}).trigger("reloadGrid");
	})
	...
	$('#del-buttion').click(function() {
		...
		if ($.trim(ids) != '') {
			$("#tables").delGridRow(ids, {
				url: SERVER_URL + '/product/category/del?nd=' + new Date().getTime(),
				mtype: 'GET',
	...
	$('#add-button').click(function() {
		dialog({
			...
			ok: false,
			onshow: function() {
				$('#categorySubmit').unbind("click").click(function(e) {
...
function pageInit() {
	$("#tables").jqGrid({
		url: SERVER_URL + '/product/category/getList?nd=' + new Date().getTime(),
		datatype: "json",
		postData: {
			category_name_search: $.trim($('#category_name_search').val())
		},
		mtype: 'post',
		colNames: ['产品类别名称', '添加时间', '操作'],
		regional: 'cn',
		colModel: [{
			name: 'category_name',
			index: 'category_name',
